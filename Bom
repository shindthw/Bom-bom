...
my_own_section:AddParagraph("Gold Bomb Jump", "Get Gold Bomb and hold it. Press again to place and jump. 4s cooldown. Drag to move button.")
...
-- Function to reset cooldown ONLY on death or plugin disable
...
-- Function to start cooldown after placing bomb
function StartCooldown()
    onCooldown = true
    -- bombEquipped durumunu bozmuyoruz, PLACE olarak kalacak
    ...
    cooldownCoroutine = coroutine.create(function()
        ...
        -- Cooldown bitti, buton PLACE olarak kalacak
        onCooldown = false
        if CircleButton and pluginEnabled and bombEquipped then
...
-- Function to get Gold Bomb (equip it)
function GetGoldBomb()
    ...
    CircleButton.BackgroundColor3 = Color3.fromRGB(255, 170, 0)
    CircleButton.Text = "PLACE"
    shared.Notify("Gold Bomb equipped! Press again to place and jump.", 2)
end

-- Function to place Gold Bomb and jump
function PlaceGoldBombAndJump()
    if not bombEquipped or not pluginEnabled then
        return
    end
    
    local GoldBomb = LocalPlayer.Character:FindFirstChild("GoldBomb")
    if GoldBomb and GoldBomb:FindFirstChild("Remote") then
        ...
        if centerCFrame then
            ...
            pcall(function()
                GoldBomb.Remote:FireServer(unpack(argsPlace))
            end)
...
my_own_section:AddToggle("Enable Gold Bomb Jump", function(bool)
    pluginEnabled = bool
    if bool then
        CreateGUI()
        SetupDragSystem()
        MonitorCharacterDeath()
        shared.Notify("Gold Bomb Jump enabled! Look for the blue squircle button.", 3)
    else
        if ScreenGui then
            ScreenGui:Destroy()
            ScreenGui = nil
        ...
        ResetCooldown()
        shared.Notify("Gold Bomb Jump disabled!", 2)
    end
end)
...
my_own_section:AddKeybind("Quick Gold Bomb", "F", function()
...
print("Gold Bomb Jump Plugin Loaded!")
